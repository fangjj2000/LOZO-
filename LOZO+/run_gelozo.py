...
class OurArguments(TrainingArguments):
    ...
    zo_eps: float = 5e-2 # eps in GELOZO
    step_u_interval: int = 50 # $\nu$ in GELOZO
    p_keep: int = 2  # retained rank in GELOZO 0 < p < r
    threshold: float = 0. # threshold for greedy update in GELOZO
...
class Framework:
    ...
    def train(self, train_samples, eval_samples):
        ...
        if self.args.trainer == "GELOZO":
            trainer = GELowRankTrainer(
                model=self.model, 
                args=self.args,
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if not args.no_eval:
                ...
                if args.local_rank <= 0:
                    write_metrics_to_file(metrics, "result/text/" +  result_file_tag(args) + f"-trainset{train_set_id}.json" if args.result_file is None else args.result_file)
