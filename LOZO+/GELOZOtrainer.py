...
class GELowRankTrainer(Trainer):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    if args.trainer == "GELOZO":
                        ...
                        self.step += 1
                        if self.step % self.args.step_u_interval == 0:
                            self.columnwiseloss(model, inputs)
                            self.greedy_update_u(self.loss_original, self.loss_update)
                    else:
                        # Gradient clipping
    ...
    def init_orthogonal_u(self):
        for name, param in self.named_parameters_to_optim:
            if param.data.ndim >= 2:
                self.u[name] = torch.nn.init.orthogonal_(torch.empty(param.data.size(0), self.args.rank_r, device=param.data.device, dtype=torch.float32)).to(param.dtype)

    def columnwiseloss(self, model, inputs):
        r = self.args.rank_r
        self.v_upd = {}
        self.loss_update = torch.empty(r, device=model.device)
        for name, param in self.named_parameters_to_optim:
            if param.data.ndim >= 2:
                self.v_upd[name] = (self.u[name].float().t() @ param.data.float()).to(param.dtype).t() - self.v_ori[name]
        for i in range(r):
            for name, param in self.named_parameters_to_optim:
                if param.data.ndim >= 2:
                    mask = torch.ones(r, dtype=torch.bool, device=param.data.device)
                    mask[i] = False
                    u = self.u[name][:, mask]
                    v = self.v_upd[name][:, mask]
                    param.data -= (u.float() @ v.float().t()).to(param.dtype)
            loss_i = self.zo_forward(model, inputs)
            self.loss_update[i] = loss_i.float()
            for name, param in self.named_parameters_to_optim:
                if param.data.ndim >= 2:
                    mask = torch.ones(r, dtype=torch.bool, device=param.data.device)
                    mask[i] = False
    ...
    def greedy_update_u(self, loss_original, loss_update):
        r = self.args.rank_r
        p_keep = self.args.p_keep
        threshold = ((1 - self.step / self.args.max_steps) * 0.1)
        torch.manual_seed(self.zo_random_seed)
        contribution = (loss_original - loss_update).float()
        print(f"[threshold] = {threshold}")
        print(f"[Step {self.step}] loss_original =", loss_original)
        print(f"[Step {self.step}] loss_update   =", loss_update)
        print(f"[Step {self.step}] contribution  =", contribution)

        must_keep = (contribution >= threshold).nonzero(as_tuple=False).flatten()

        if len(must_keep) > p_keep:
            top_idx = torch.topk(contribution[must_keep], k=p_keep, largest=True).indices
            keep_idx = must_keep[top_idx]
        else:
            keep_idx = must_keep
        for name, param in self.named_parameters_to_optim:
            # layer_seed = self.zo_random_seed + (hash(name) % 100000)
            # torch.manual_seed(layer_seed)
            if param.data.ndim >= 2:
                m, r = self.u[name].shape
                u_keep = self.u[name][:, keep_idx]
                k = r - len(keep_idx)
                if k > 0:
                    A = torch.randn((m, k), device=param.data.device, dtype=torch.float32)
                    u_keep32 = u_keep.to(torch.float32)
                    A -= u_keep32 @ (u_keep32.T @ A)
                    u_new32, _ = torch.linalg.qr(A, mode="reduced")
                    u_new = u_new32.to(param.data.dtype)
                    u_next = torch.cat([u_keep, u_new], dim=1)
                else:
                    u_next = u_keep
                self.u[name] = u_next
    ...
    def lowrank_zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            if param.data.ndim >= 2:
                self.v[name] = torch.randn(param.data.size(1), args.rank_r, device=param.data.device, dtype=param.data.dtype)
                param.data = param.data + (scaling_factor * (self.u[name].float() @ self.v[name].float().t()) * self.args.zo_eps).to(param.dtype)
            # else:
            #     z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)/100
    ...
    def lowrank_zo_step(self, model, inputs):
        """
        Estimate gradient by Lowrank-zo. Return the loss from f(theta + e*uv^t)
        """
        args = self.args
        if self.step==0:
            self.u = {}
            self.v = {}
            self.v_ori = {}
            self.init_orthogonal_u()

        if self.step % args.step_u_interval == 0:
            self.loss_original = self.zo_forward(model, inputs)
            for name, param in self.named_parameters_to_optim:
                if param.data.ndim >= 2:
                    self.v_ori[name] = (self.u[name].float().t()@param.data.float()).t().to(param.dtype)
        ...
        for name, param in self.named_parameters_to_optim:
            if param.data.ndim >= 2:
                ...
                if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                    param.data = param.data - (self._get_learning_rate() * (self.projected_grad * (u.float()@v.float().t()) + args.weight_decay * param.data)/self.args.rank_r).to(param.dtype)  #是否需要除以 r? LOZO算法里面有，但没除.
                else:
                    param.data = param.data - (self._get_learning_rate() * (self.projected_grad * (u.float()@v.float().t()))/self.args.rank_r).to(param.dtype)
            # else:
            #     # Resample z for bias
